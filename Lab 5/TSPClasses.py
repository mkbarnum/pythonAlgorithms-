...
class Node:
    def __init__(self, lowerBound, costMatrix, path, depth):
    ...
    def getPriority(self):
        return self._lowerBound / self._depth

    def getCitiesToVisit(self):
        fromCity = self._path[len(self._path) - 1]
        citiesToVisit = []
        for i in range(len(self._costMatrix[0])):
            if self._costMatrix[fromCity, i] != math.inf:
                citiesToVisit.append(i)

        return citiesToVisit

    def calculateMatrix(self, parentMatrix, parentLB, toCity, BSSF):
        fromCity = self._path[len(self._path) - 2]
        parentLB = parentLB + parentMatrix[fromCity, toCity]
        parentMatrix[fromCity] = math.inf
        parentMatrix[:, toCity] = math.inf
        parentMatrix[toCity, fromCity] = math.inf

        length = len(parentMatrix[0])
        for i in range(0, length):
            minValue = min(parentMatrix[i])
            if minValue > 0 and minValue != math.inf:
                parentMatrix[i, 0:length+1] -= minValue
                parentLB += minValue
                if parentLB >= BSSF:
                    self._lowerBound = math.inf
                    return

        for j in range(0, length):
            minValue = min(parentMatrix[:, j])
            if minValue > 0 and minValue != math.inf:
                parentMatrix[0:length+1, j] -= minValue
                parentLB += minValue
                if parentLB >= BSSF:
                    self._lowerBound = math.inf
                    return

        self._lowerBound = parentLB
        self._costMatrix = parentMatrix

    def connectsCity(self):
        lastCity = self._path[len(self._path) - 1]
        startCity = self._path[0]
        if self._costMatrix[lastCity, startCity] != math.inf:
            return True
        else:
            return False
